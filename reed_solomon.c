...
reed_solomon_ctx *reed_solomon_ctx_new(uint64_t needed_chunk_bytelen, uint64_t data_len){
    ...
    BIGNUM *field = BN_CTX_get(bn_ctx);
    ...
    {
        rs_ctx->data[i] = BN_CTX_get(bn_ctx);
...
void set_data_at(reed_solomon_ctx *rs_ctx, uint8_t *data, uint64_t data_index) {
    ...
    for (uint64_t j = 0; j < i; ++j) {
        ...
        BN_GF2m_mod_mul(rs_ctx->lagrange[j], rs_ctx->lagrange[j], temp, rs_ctx->field, rs_ctx->bn_ctx);
    }
    BN_free(temp);
...
void compute_chunk_at(reed_solomon_ctx *rs_ctx, uint64_t chunk_index, uint8_t *chunk_bytes)
{
    ...
    for (uint64_t i = 0; i < rs_ctx->data_len; ++i) {
        ...
        BN_GF2m_mod_mul(lagrange, rs_ctx->data[i], rs_ctx->lagrange[i], rs_ctx->field, rs_ctx->bn_ctx);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        printf("usage: ./main <chunk_bytes> <base_length>");
        return 0;
    }
    ...
    reed_solomon_ctx *rs_ctx = reed_solomon_ctx_new(chunk_byte_len, base_length);
    ...
    reed_solomon_ctx *decoder_ctx = reed_solomon_ctx_new(chunk_byte_len, base_length);

    // Set non-base data
    printf("Compute non-base data %ld chunks...\n", base_length);
    start = clock();

    uint64_t nonbase_shift = base_length;
    uint8_t **nonbase = calloc(base_length, sizeof(uint8_t*));
    for (uint64_t i = 0; i < base_length; ++i)
    {
    ...
    assert(decoder_ctx->data_len == base_length);
